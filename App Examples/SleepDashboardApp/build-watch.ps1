...
$packageLog = ($log -split "`n" | Where-Object { $_ -match [regex]::Escape($Package) }) -join "`n"

if ($packageLog -match "FATAL EXCEPTION|AndroidRuntime.*FATAL") {
    $errors += "FATAL EXCEPTION in logcat (app crashed on launch)"
}
if ($packageLog -match "ANR in $Package|Application Not Responding") {
    $errors += "ANR detected for $Package"
}
if ($packageLog -match "ClassNotFoundException") {
    $errors += "ClassNotFoundException in logcat for $Package (missing class - invented API or missing module)"
}
if ($packageLog -match 'Resources\.NotFoundException') {
    $errors += "Resources.NotFoundException in logcat for $Package (missing asset - FileN wiring mismatch)"
}
