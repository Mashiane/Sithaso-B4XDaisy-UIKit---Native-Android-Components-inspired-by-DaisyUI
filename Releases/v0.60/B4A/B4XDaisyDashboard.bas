...
Sub Class_Globals
	...
	Private mActiveIndicatorColor As Int = 0xFF3B82F6 ' overridden by theme in ApplyDesignerProps
...
' helper to avoid Nulls when trimming/reading text
...
Public Sub AddToParent(Parent As B4XView)
	...
	Dim w As Int = B4XDaisyVariants.ResolveSizeSpec(mWidthSpec, Parent.Width, Parent.Width)
	Dim h As Int = B4XDaisyVariants.ResolveSizeSpec(mHeightSpec, Parent.Height, Parent.Height)
...
' Adds a single dashboard button using id, label and an SVG asset/full path.
Public Sub AddButtonWithSvgPath(Id As String, Label As String, FullSvgPath As String) As Boolean
	If Buttons.IsInitialized = False Then Buttons.Initialize
	Dim normalizedId As String = SafeString(Id).Trim
	If normalizedId.Length = 0 Then Return False
	If FindButtonIndexById(normalizedId) >= 0 Then Return False
	...
	Dim item As Map = CreateMap( _
		...
		"label": SafeString(Label), _
		"imagePath": "", _
		"svgPath": SafeString(FullSvgPath).Trim _
	)
	Buttons.Add(NormalizeButton(item, Buttons.Size))
	BuildPages
	Return True
...
Public Sub setWidth(Value As Object)
	mWidthSpec = B4XDaisyVariants.NormalizeSizeSpec(Value, "100%")
	ApplySizeFromParent
	BuildPages
End Sub
...
Public Sub setHeight(Value As Object)
	mHeightSpec = B4XDaisyVariants.NormalizeSizeSpec(Value, "100%")
	ApplySizeFromParent
	BuildPages
End Sub
...
Private Sub CreateInternalViews
	...
	#If B4A
	' Suppress Android scrollbars/overscroll glow that show as a bottom line near indicators.
	Dim joHsv As JavaObject = hsv
	joHsv.RunMethod("setHorizontalScrollBarEnabled", Array(False))
	joHsv.RunMethod("setVerticalScrollBarEnabled", Array(False))
	joHsv.RunMethod("setOverScrollMode", Array(2)) ' OVER_SCROLL_NEVER
	#End If
...
Private Sub LoadBitmapFromPath(Path As String) As B4XBitmap
	...
	Try
		...
		Dim isSvg As Boolean = p.ToLowerCase.EndsWith(".svg")
		If slash >= 0 Then
			...
			If dir.Length > 0 And fn.Length > 0 And File.Exists(dir, fn) Then
				If isSvg Then
					Dim svgText As String = File.ReadString(dir, fn)
					Dim svgBmp As B4XBitmap = RenderSvgBitmap(svgText)
					If svgBmp.IsInitialized Then Return svgBmp
				End If
		...
		If File.Exists(File.DirAssets, p) Then
			If isSvg Then
				Dim svgTextAsset As String = File.ReadString(File.DirAssets, p)
				Dim svgBmpAsset As B4XBitmap = RenderSvgBitmap(svgTextAsset)
				If svgBmpAsset.IsInitialized Then Return svgBmpAsset
			End If
...
Private Sub RenderSvgBitmap(SvgText As String) As B4XBitmap
	Dim empty As B4XBitmap
	Dim content As String = SvgText
	If content = Null Then Return empty
	content = content.Trim
	If content.Length = 0 Then Return empty
	Try
		Dim iconRenderer As B4XDaisySvgIcon
		iconRenderer.Initialize(Me, "")
		Dim jo As JavaObject = iconRenderer				'ignore
		Dim tintColor As Int = B4XDaisyVariants.GetTokenColor("--color-base-200", mTextColor)
		Dim o As Object = jo.RunMethod("nativeRenderSvgToBitmap", Array(content, 128dip, 128dip, tintColor, False))
		If o <> Null Then
...
Private Sub ApplyDesignerProps(Props As Map)
	...
	mMinCellWidth = B4XDaisyVariants.GetPropDip(Props, "MinCellWidth", 72dip)
	mMinCellHeight = B4XDaisyVariants.GetPropDip(Props, "MinCellHeight", 100dip)
	mPagePadding = B4XDaisyVariants.GetPropDip(Props, "PagePadding", 16dip)
	mCellSpacing = B4XDaisyVariants.GetPropDip(Props, "CellSpacing", 12dip)
	mCellSpacingY = B4XDaisyVariants.GetPropDip(Props, "CellSpacingY", 0dip)
	mGridTopOffset = B4XDaisyVariants.GetPropDip(Props, "GridTopOffset", 12dip)
	mIndicatorAreaHeight = B4XDaisyVariants.GetPropDip(Props, "IndicatorAreaHeight", 26dip)
	mIndicatorSize = B4XDaisyVariants.GetPropDip(Props, "IndicatorSize", 6dip)
	mIndicatorGap = B4XDaisyVariants.GetPropDip(Props, "IndicatorGap", 8dip)
	mActiveIndicatorColor = B4XDaisyVariants.GetPropColor(Props, "ActiveIndicatorColor", 0xFF3B82F6)
	mInactiveIndicatorColor = B4XDaisyVariants.GetPropColor(Props, "InactiveIndicatorColor", 0x553B82F6)
	mBackgroundImagePath = B4XDaisyVariants.GetPropString(Props, "BackgroundImage", "")
	mTextColor = B4XDaisyVariants.GetPropColor(Props, "TextColor", 0xFFFFFFFF)
	mWidthSpec = B4XDaisyVariants.GetPropString(Props, "Width", "100%")
	mHeightSpec = B4XDaisyVariants.GetPropString(Props, "Height", "100%")
	
	' Apply theme if any property is 0/default
	' (Dashboard usually just uses explicit colors though)
End Sub
...
Private Sub ApplySizeFromParent
	...
	Dim targetW As Int = B4XDaisyVariants.ResolveSizeSpec(mWidthSpec, parent.Width, mBase.Width)
	Dim targetH As Int = B4XDaisyVariants.ResolveSizeSpec(mHeightSpec, parent.Height, mBase.Height)
