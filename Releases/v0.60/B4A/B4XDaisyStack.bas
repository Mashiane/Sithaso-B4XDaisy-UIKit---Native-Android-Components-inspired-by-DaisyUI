...
Public Sub setWidth(Value As Object)
	mWidth = Max(16dip, B4XDaisyVariants.TailwindSizeToDip(Value, B4XDaisyVariants.ResolveWidthBase(mBase, mWidth)))
	mWidthExplicit = True
	If mBase.IsInitialized = False Then Return
...
Private Sub ApplyDesignerProps(Props As Map)
	mWidth = Max(16dip, GetPropSizeDip(Props, "Width", "w-10"))
	mHeight = Max(16dip, GetPropSizeDip(Props, "Height", "h-10"))
	mPadding = B4XDaisyVariants.GetPropString(Props, "Padding", "")
	mMargin = B4XDaisyVariants.GetPropString(Props, "Margin", "")
	mDirection = NormalizeDirection(B4XDaisyVariants.GetPropString(Props, "Direction", "bottom"))
	mStepPrimary = Max(0, GetPropDip(Props, "StepPrimary", 7))
	mStepSecondary = Max(0, GetPropDip(Props, "StepSecondary", 3))
	If mStepSecondary > mStepPrimary Then mStepSecondary = mStepPrimary
	mAutoFillLayers = B4XDaisyVariants.GetPropBool(Props, "AutoFillLayers", True)
	mLayoutAnimationMs = Max(0, B4XDaisyVariants.GetPropInt(Props, "LayoutAnimationMs", 0))
	mRoundedBox = B4XDaisyVariants.GetPropBool(Props, "RoundedBox", False)
	mStrictDaisyParity = B4XDaisyVariants.GetPropBool(Props, "StrictDaisyParity", True)
	ApplyRoundedBox
End Sub
...
' removed simple GetPropString helper; use variant call directly
...
' removed simple GetPropBool helper; use variant call directly
