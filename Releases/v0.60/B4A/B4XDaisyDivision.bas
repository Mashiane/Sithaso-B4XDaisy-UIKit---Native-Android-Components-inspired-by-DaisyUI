...
#DesignerProperty: Key: AutoResize, DisplayName: Auto Resize, FieldType: Boolean, DefaultValue: False, Description: Automatically resize height to fit content. Text-only divisions grow like a paragraph (height:auto). Divisions with child views fit the bottom-most child.
...
Sub Class_Globals
	...
	Private mSkeletonBaseColor As Int = xui.Color_RGB(209, 213, 219)				'ignore
	Private mSkeletonShimmerColor As Int = xui.Color_RGB(243, 244, 246)				'ignore
...
Private Sub ApplyDesignerProps(Props As Map)
	...
	mWidth = Max(1dip, GetPropSizeDip(Props, "Width", "w-10"))
	Dim heightSpec As String = B4XDaisyVariants.GetPropString(Props, "Height", "h-10").ToLowerCase.Trim
	If heightSpec = "auto" Or heightSpec = "h-auto" Then
		mbAutoResize = True
		mHeightExplicit = False
	Else
		mHeight = Max(1dip, GetPropSizeDip(Props, "Height", "h-10"))
    ...
    mBackgroundColor = B4XDaisyVariants.GetPropColor(Props, "BackgroundColor", 0x00FFFFFF)
    mTextColor = B4XDaisyVariants.GetPropColor(Props, "TextColor", 0xFF000000)
    mTextSizeSpec = B4XDaisyVariants.GetPropString(Props, "TextSize", "text-sm")
    mTextSize = ResolveTextSize(mTextSizeSpec)
    ...
    mShadow = B4XDaisyVariants.NormalizeShadow(B4XDaisyVariants.GetPropString(Props, "Shadow", "none"))
     mPlaceContentCenter = B4XDaisyVariants.GetPropBool(Props, "PlaceContentCenter", False)
     mBorderWidth = B4XDaisyVariants.GetPropInt(Props, "BorderWidth", 0)
     mBorderColor = B4XDaisyVariants.GetPropColor(Props, "BorderColor", 0xFF000000)
     mBorderStyle = NormalizeBorderStyle(B4XDaisyVariants.GetPropString(Props, "BorderStyle", "solid"))
     mBorderReliefStrength = ClampReliefStrength(B4XDaisyVariants.GetPropInt(Props, "BorderReliefStrength", 55))
     mAutoReliefByStyle = B4XDaisyVariants.GetPropBool(Props, "AutoReliefByStyle", True)
...
' Returns the pixel height needed to fully display wrapped text in the given content width.
Private Sub GetPreferredHeight(ContentWidth As Int) As Int
	If lblContent.IsInitialized = False Then
		Return Max(1dip, (mTextSize * 1.5) + 8dip)
	End If
	...
	lbl.Width = savedW
	Dim bm As Map = BuildBoxModel
	Dim padV As Float = bm.GetDefault("padding_top", 0) + bm.GetDefault("padding_bottom", 0)
	Return Max(1dip, h + padV + 4dip)
End Sub

' Resizes mBase to fit content. Two modes:
'   - Text-only (no external child views): height wraps text using CSS line-height math.
'   - Has external children: height = bottom of deepest child + bottom padding.
' Does NOT call Base_Resize to avoid recursion.
Private Sub DoAutoResize
	If mBase.IsInitialized = False Then Return
	If lblContent.IsInitialized = False Then Return
	' Detect external children (anything that is not lblContent or pnlSkeleton)
	Dim externalBottom As Int = 0
	Dim hasExternal As Boolean = False
	Dim i As Int
	For i = 0 To mBase.NumberOfViews - 1
		Dim v As B4XView = mBase.GetView(i)
		Dim isInternal As Boolean = (v = lblContent)
		If isInternal = False Then
			If pnlSkeleton.IsInitialized Then
				If v = pnlSkeleton Then isInternal = True
			End If
		End If
		If isInternal = False Then
			hasExternal = True
			Dim childBottom As Int = v.Top + v.Height
			If childBottom > externalBottom Then externalBottom = childBottom
		End If
	Next
	Dim bm As Map = BuildBoxModel
	Dim newH As Int
	If hasExternal Then
		Dim padBottom As Float = bm.GetDefault("padding_bottom", 0)
		Dim marginBottom As Float = bm.GetDefault("margin_bottom", 0)
		newH = Max(16dip, externalBottom + padBottom + marginBottom)
	Else
		Dim contentW As Int = Max(1dip, lblContent.Width)
		newH = GetPreferredHeight(contentW)
	End If
	...
	Dim outerRect As B4XRect = B4XDaisyBoxModel.ResolveOuterRect(host, bm)
	Dim contentRect As B4XRect = B4XDaisyBoxModel.ResolveContentRect(outerRect, bm)
