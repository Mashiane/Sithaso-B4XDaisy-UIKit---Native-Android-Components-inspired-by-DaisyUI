...
Private Sub ApplyDesignerProps(Props As Map)
	...
	mThickness = B4XDaisyVariants.GetPropString(Props, "Thickness", "10%")
	mVariant = B4XDaisyVariants.NormalizeVariant(B4XDaisyVariants.GetPropString(Props, "Variant", "none"))
	mDisplayType = B4XDaisyVariants.GetPropString(Props, "DisplayType", "text").ToLowerCase.Trim
	mText = B4XDaisyVariants.GetPropString(Props, "Text", "0")
	mPrefix = B4XDaisyVariants.GetPropString(Props, "Prefix", "")
	mSuffix = B4XDaisyVariants.GetPropString(Props, "Suffix", "%")
	mTextCountUp = B4XDaisyVariants.GetPropBool(Props, "TextCountUp", False)
	mCountUpSpeed = B4XDaisyVariants.GetPropInt(Props, "CountUpSpeed", 300)
	mSvgAsset = B4XDaisyVariants.GetPropString(Props, "SvgAsset", "")
	...
	mBorderWidth = B4XDaisyVariants.GetPropString(Props, "BorderWidth", "0")
...
Public Sub SetValueAnimated(NewValue As Float, Duration As Int)
	...
	If Duration <= 0 Then
		...
		DrawComponent
	Else
		If AnimationTimer.IsInitialized = False Then
			AnimationTimer.Initialize("AnimationTimer", 16)
		End If
...
Private Sub AnimationTimer_Tick
	Dim t As Long = DateTime.Now - AnimationStartTime
...
Public Sub StopAnimation
	If AnimationTimer.IsInitialized Then
		AnimationTimer.Enabled = False
	End If
End Sub
...
Public Sub setStepValue(StepVal As Int)
	mStepValue = Max(1, StepVal)
	DrawComponent
End Sub

Public Sub getStepValue As Int
	Return mStepValue
End Sub
...
Public Sub getThickness As String
	Return mThickness
End Sub
...
Public Sub getTextCountUp As Boolean
	Return mTextCountUp
End Sub
...
Public Sub getCountUpSpeed As Int
	Return mCountUpSpeed
End Sub
...
Public Sub getTrackColor As Int
	Return mTrackColor
End Sub
