...
Public Sub Base_Resize (Width As Double, Height As Double)
	...
	Dim pct As Float = mValue / range
	' Safety check for NaN or Infinity
	If IsNumber(pct) = False Then pct = 0
	pct = Max(0, Min(1, pct))
	...
	pnlValue.SetLayoutAnimated(0, 0, 0, Max(0, barW), h)
	tipAnchor.SetLayoutAnimated(0, Max(0, barW), 0, 1dip, h)
