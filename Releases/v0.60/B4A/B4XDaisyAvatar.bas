...
Private Sub ResolveOnlineColor As Int
	...
	If mbUseVariantStatusColors Then c = B4XDaisyVariants.ResolveVariantColor(ActivePalette, msVariant, "back", c)
...
Private Sub ResolveOfflineColor As Int
	...
	If mbUseVariantStatusColors Then
		c = B4XDaisyVariants.ResolveVariantColor(ActivePalette, msVariant, "muted", c)
		If c = 0 Then c = B4XDaisyVariants.ResolveOfflineColor(msVariant, AvatarOfflineColor)
	End If
...
Private Sub DrawAvatar
	...
	If effectiveStatus <> "none" Then
		Dim dd As Float = IIf(mbChatImage, Min(aw, ah), Min(imageRect.Width, imageRect.Height))
