...
#DesignerProperty: Key: Width, DisplayName: Width, FieldType: String, DefaultValue: w-6, Description: Tailwind size token or CSS size (eg w-6, 24px, 2rem)
#DesignerProperty: Key: Height, DisplayName: Height, FieldType: String, DefaultValue: h-6, Description: Tailwind size token or CSS size (eg h-6, 24px, 2rem)
...
Sub Class_Globals
	...
	Private mPreserveColors As Boolean = False
...
Private Sub ApplyDesignerProps(Props As Map)
	mWidth = Max(1dip, GetPropSizeDip(Props, "Width", "w-6"))
	mHeight = Max(1dip, GetPropSizeDip(Props, "Height", "h-6"))
	IconColor = B4XDaisyVariants.GetPropColor(Props, "Color", B4XDaisyVariants.GetTokenColor("--color-accent", 0xFF3B82F6))
	mPreserveColors = B4XDaisyVariants.GetPropBool(Props, "PreserveColors", False)
	mPadding = Max(0, GetPropDip(Props, "Padding", 0))
	mBorderWidth = Max(0, GetPropDip(Props, "BorderWidth", 0))
	mBorderColor = B4XDaisyVariants.GetPropColor(Props, "BorderColor", 0x00000000)
	mBackgroundColor = B4XDaisyVariants.GetPropColor(Props, "BackgroundColor", 0x00000000)
	...
	setSvgAsset(B4XDaisyVariants.GetPropString(Props, "SvgAsset", ""))
	Dim v As String = B4XDaisyVariants.NormalizeVariant(B4XDaisyVariants.GetPropString(Props, "Variant", "none"))
	If v <> "none" Then
...
Private Sub RenderSvg
	If ivNative.IsInitialized = False Then Return
	If SvgMarkup.Trim.Length = 0 And SvgAssetPath.Trim.Length = 0 Then
...
Private Sub TryRenderNative(Svg As String) As Boolean
	...
	Try
		...
		Dim o As Object = jo.RunMethod("nativeRenderSvgToBitmap", Array(Svg, w, h, IconColor, mPreserveColors))
...
Private Sub GetResolvedSvgContent As String
	...
	If raw.Length = 0 And SvgAssetPath.Length > 0 Then raw = B4XDaisyApp.GetCachedSvgText(SvgAssetPath, "").Trim
...
' removed forwarding helpers; call variant methods directly
...
private static boolean drawElement(Canvas canvas, Path path, int tintColor, boolean preserveColors, Map<String, String> style, float opacityMul) {
	...
	int fillColor = preserveColors ? parseColor(fillValue, currentColor, currentColor) : tintColor;
		...
		switch (u) {
			...
			case 'A': {
				while (sc.hasNumber()) {
					...
					int laf = sc.nextArcFlag();
					int sf = sc.nextArcFlag();
					float x = sc.nextFloat(), y = sc.nextFloat();
					if (rel) { x += currentX; y += currentY; }
					if (!addSimpleArc(out, currentX, currentY, x, y, rx, ry, angle, laf != 0, sf != 0)) {
						out.lineTo(x, y);
					}
					currentX = x; currentY = y;
					lastC2X = currentX; lastC2Y = currentY; lastQX = currentX; lastQY = currentY;
...
private static boolean addSimpleArc(Path path, float x0, float y0, float x1, float y1, float rx, float ry, float rotation, boolean largeArc, boolean sweep) {
	...
	if (Math.abs(x0 - x1) < 0.0001f && Math.abs(y0 - y1) < 0.0001f) return true;
	rx = Math.abs(rx);
	ry = Math.abs(ry);
	if (rx <= 0f || ry <= 0f) return false;
	drawArc(path, x0, y0, x1, y1, rx, ry, rotation, largeArc, sweep);
	return true;
}

private static void drawArc(Path path, float x0, float y0, float x1, float y1, float rx, float ry, float angle, boolean largeArc, boolean sweep) {
	double theta = Math.toRadians(angle);
	double cosTheta = Math.cos(theta);
	double sinTheta = Math.sin(theta);
	double x0p = (x0 * cosTheta + y0 * sinTheta) / rx;
	double y0p = (-x0 * sinTheta + y0 * cosTheta) / ry;
	double x1p = (x1 * cosTheta + y1 * sinTheta) / rx;
	double y1p = (-x1 * sinTheta + y1 * cosTheta) / ry;
	double dx = x0p - x1p;
	double dy = y0p - y1p;
	...
	if (disc < 0.0) {
		double scale = Math.sqrt(dsq) / 1.99999;
		drawArc(path, x0, y0, x1, y1, (float) (rx * scale), (float) (ry * scale), angle, largeArc, sweep);
		return;
	}
	...
	double eta0 = Math.atan2(y0p - cyp, x0p - cxp);
	double eta1 = Math.atan2(y1p - cyp, x1p - cxp);
	double sweepAngle = eta1 - eta0;
	if (sweep != (sweepAngle >= 0.0)) {
		sweepAngle += (sweepAngle > 0.0) ? -2.0 * Math.PI : 2.0 * Math.PI;
	}

	double cx = cxp * rx;
	double cy = cyp * ry;
	double centerX = cx * cosTheta - cy * sinTheta;
	double centerY = cx * sinTheta + cy * cosTheta;
	arcToBezier(path, centerX, centerY, rx, ry, theta, eta0, sweepAngle);
}

private static void arcToBezier(Path path, double cx, double cy, double rx, double ry, double theta, double start, double sweep) {
	int segments = (int) Math.ceil(Math.abs(sweep * 4.0 / Math.PI));
	if (segments <= 0) return;
	double cosTheta = Math.cos(theta);
	double sinTheta = Math.sin(theta);
	double eta1 = start;
	double e1x = cx + rx * cosTheta * Math.cos(eta1) - ry * sinTheta * Math.sin(eta1);
	double e1y = cy + rx * sinTheta * Math.cos(eta1) + ry * cosTheta * Math.sin(eta1);
	double ep1x = -rx * cosTheta * Math.sin(eta1) - ry * sinTheta * Math.cos(eta1);
	double ep1y = -rx * sinTheta * Math.sin(eta1) + ry * cosTheta * Math.cos(eta1);
	double anglePerSegment = sweep / segments;

	for (int i = 0; i < segments; i++) {
		double eta2 = eta1 + anglePerSegment;
		double sinEta2 = Math.sin(eta2);
		double cosEta2 = Math.cos(eta2);
		double e2x = cx + rx * cosTheta * cosEta2 - ry * sinTheta * sinEta2;
		double e2y = cy + rx * sinTheta * cosEta2 + ry * cosTheta * sinEta2;
		double ep2x = -rx * cosTheta * sinEta2 - ry * sinTheta * cosEta2;
		double ep2y = -rx * sinTheta * sinEta2 + ry * cosTheta * cosEta2;
		...
		path.cubicTo((float) q1x, (float) q1y, (float) q2x, (float) q2y, (float) e2x, (float) e2y);
		eta1 = eta2;
		e1x = e2x;
		e1y = e2y;
		ep1x = ep2x;
