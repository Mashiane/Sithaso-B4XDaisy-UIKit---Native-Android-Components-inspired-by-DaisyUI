...
Sub Process_Globals
	...
	Public Const LINE_HEIGHT_DEFAULT As Float = 1.5
End Sub
...
' Set overflow hidden on a view
Public Sub SetOverflowHidden(v As B4XView)
	' Clip to outline
	Dim jo As JavaObject = v
	jo.RunMethod("setClipToOutline", Array(True))
End Sub

' Set a CSS/Style variable on a view (B4A only for now)
Public Sub SetStyleVariable(v As B4XView, Name As String, Value As Object)
    #If B4A
    Try
        Dim jo As JavaObject = v
        ' In SNH (SithasoNewHeights) and DaisyUIKit, we often store these in the view's tag 
        ' or a dedicated map if it's a composite component. 
        ' For simple CSS variables that the underlying engine (like a WebView or custom renderer) 
        ' might need, we can use setTag or a custom property.
        ' However, for Daisy rotation, these are actually used by the B4X Transition or 
        ' the component's Refresh logic.
        ' We'll store it in a Map in the Tag if the Tag is a Map, otherwise we'll just 
        ' ensure it's available for the component.
        If v.Tag <> Null And v.Tag Is Map Then
            Dim m As Map = v.Tag
            m.Put(Name, Value)
        End If
    Catch
...
' Check if an object is of a specific class name
Public Sub IsClass(Obj As Object, ClassName As String) As Boolean
    If Obj = Null Then Return False
    Dim sType As String = GetType(Obj).ToLowerCase
    Return sType.Contains(ClassName.ToLowerCase)
End Sub
...
Public Sub NormalizeOrientation(Value As String) As String
...
Public Sub NormalizeHorizontalPlacement(Value As String) As String
...
Public Sub NormalizeVerticalPlacement(Value As String) As String
...
Public Sub NormalizeAlertVariant(Value As String) As String
	If Value = Null Then Return "none"
	Dim s As String = Value.ToLowerCase.Trim
	If s.StartsWith("alert-") Then s = s.SubString(6)
	Select Case s
		Case "none", "info", "success", "warning", "error", "neutral", "primary", "secondary", "accent"
			Return s
		Case Else
			Return "none"
	End Select
...
Public Sub NormalizeSize(Value As String) As String
...
Public Sub NormalizeBadgeStyle(Value As String) As String
...
Public Sub NormalizeStyle(Value As String) As String
	If Value = Null Then Return "solid"
	Dim s As String = Value.ToLowerCase.Trim
	Select Case s
...
Public Sub NormalizeSelectionMode(Value As String) As String
...
Public Sub NormalizeFieldsetBorderStyle(Value As String) As String
...
Public Sub NormalizeLegendSize(Value As String) As String
...
Public Sub NormalizeSizeSpec(Value As Object, DefaultValue As String) As String
	Dim s As String = ""
	If Value <> Null Then s = Value
	s = s.Trim
	If s.Length = 0 Then Return DefaultValue
	Return s
End Sub

Public Sub GetPropObject(Props As Map, Key As String, DefaultValue As Object) As Object
...
Public Sub ResolveSizeSpec(Value As String, ParentSize As Int, Fallback As Int) As Int
	Dim s As String = IIf(Value = Null, "", Value.Trim)
	If s.Length = 0 Then Return Max(1dip, Fallback)
	' Strip Tailwind bracket prefix so w-[80%], h-[80%], and [80%] resolve as % of ParentSize.
	Dim lower As String = s.ToLowerCase
	If lower.StartsWith("w-[") And lower.EndsWith("]") Then
		s = s.SubString2(3, s.Length - 1)
	Else If lower.StartsWith("h-[") And lower.EndsWith("]") Then
		s = s.SubString2(3, s.Length - 1)
	Else If lower.StartsWith("[") And lower.EndsWith("]") And lower.Length > 2 Then
	...
	If s.EndsWith("%") Then
		...
		End If
	End If
	Return Max(1dip, TailwindSizeToDip(s, Fallback))
End Sub

Public Sub NormalizeAvatarPosition(Value As String) As String
...
Public Sub NormalizeDirection(Value As String) As String
	If Value = Null Then Return "horizontal"
	Dim s As String = Value.ToLowerCase.Trim
	Select Case s
...
Public Sub NormalizeAnimation(Value As String) As String
	If Value = Null Then Return "none"
	Dim s As String = Value.ToLowerCase.Trim
	Select Case s
...
Public Sub NormalizeSingleLineText(Value As String) As String
	If Value = Null Then Return ""
	Return Value.Replace(CRLF, " ").Replace(Chr(10), " ").Replace(Chr(13), " ").Trim
End Sub
...
' helper used by demo pages
Public Sub GetThemeColor(Token As String, DefaultColor As Int) As Int
	' try to get the numeric value of the token; fall back to the provided default
	Return GetTokenNumber(Token, DefaultColor)
End Sub
...
Private Sub RegisterLightTheme
	...
	light.Put("--color-base-300", 0xFFDEDBEB)
...
''' Resolves a variant name to its text-[variant] color.
''' "primary" → "text-primary" → --color-primary (the variant hue itself, not -content).
''' Use this when the variant color is applied AS text color (e.g. ValueColor, FigureVariant).
Public Sub ResolveTextColor(VariantName As String, DefaultColor As Int) As Int
	If VariantName = Null Then Return DefaultColor
	Dim name As String = VariantName.Trim
	If name.Length = 0 Or name = "none" Then Return DefaultColor
	Dim token As String = ResolveThemeColorTokenName("text-" & name)
	If token.Length > 0 Then Return GetTokenColor(token, DefaultColor)
	Return DefaultColor
End Sub
...
Public Sub ResolveColorVariantFromPalette(Palette As Map, VariantOrToken As String, PaletteKey As String, DefaultColor As Int) As Int
	...
	' "primary-text", "error-text", etc. → variant's own hue (not -content).
	If normalizedName.EndsWith("-text") Then
		Dim baseName As String = normalizedName.SubString2(0, normalizedName.Length - 5)
		Return ResolveTextColor(baseName, DefaultColor)
	End If

	' "link-success", "link-primary", etc. → strip "link-" prefix → variant hue.
	If normalizedName.StartsWith("link-") Then
		Dim baseName As String = normalizedName.SubString(5)
		Return ResolveTextColor(baseName, DefaultColor)
	End If

	' "text-success", "text-primary", etc. → strip "text-" prefix → variant hue.
	If normalizedName.StartsWith("text-") Then
		Dim baseName As String = normalizedName.SubString(5)
		Return ResolveTextColor(baseName, DefaultColor)
	End If
...
Public Sub NormalizeMask(MaskName As String) As String
	...
	If m = "none" Then Return "rounded-none"
	Select Case m
...
' Sets the line spacing multiple (line-height) for a view (Android TextView).
' Multiple: 1.0 is default, 1.5 is standard DaisyUI spacing.
' Add: extra pixels to add between lines (usually 0).
' Includes parity fix: setIncludeFontPadding(False) to remove Android's top/bottom spacing.
Public Sub SetLineSpacing(v As B4XView, Multiple As Float, Add As Float)
	#If B4A
	' Guard: only TextView/Label supports setIncludeFontPadding and setLineSpacing.
	' Panels (BALayout) do not. Use Try/Catch so callers passing a Panel are silently ignored.
	Try
		Dim jo As JavaObject = v
		jo.RunMethod("setIncludeFontPadding", Array(False))
		jo.RunMethod("setLineSpacing", Array(Add, Multiple))
	Catch
		' View is not a TextView — skip line spacing silently.
	End Try				'ignore
	#Else
	Dim ignoreV As Object = v
	Dim ignoreM As Float = Multiple
	Dim ignoreA As Float = Add
	#End If
End Sub
...
Public Sub GetPropColor(Props As Map, Key As String, DefaultValue As Int) As Int
	' Safely handle numeric colors and string tokens without causing a ClassCastException.
	If Props.IsInitialized = False Then Return DefaultValue
	If Props.ContainsKey(Key) = False Then Return DefaultValue
	Dim o As Object = Props.Get(Key)
	Return ResolveColorValue(o, DefaultValue)
End Sub

' ELI5: This converts numbers or color tokens into a real color value.
Public Sub ResolveColorValue(Value As Object, DefaultColor As Int) As Int
	If Value = Null Then Return DefaultColor
	If IsNumber(Value) Then Return Value
	Dim s As String = Value
	s = s.Trim
	If s.Length = 0 Then Return DefaultColor
	Dim token As String = ResolveThemeColorTokenName(s)
	If token.Length > 0 Then Return GetTokenColor(token, DefaultColor)
	Return DefaultColor
End Sub

Public Sub GetPropDip(Props As Map, Key As String, DefaultDip As Float) As Float
	If Props.IsInitialized = False Then Return DefaultDip
	If Props.ContainsKey(Key) = False Then Return DefaultDip
	Dim o As Object = Props.Get(Key)
	Return TailwindSizeToDip(o, DefaultDip)
End Sub


Public Sub GetPropSizeDip(Props As Map, Key As String, DefaultDip As Object) As Float
	' Accept either a numeric default or a size token string (e.g. "full", "12", "w-40").
	Dim def As Float
	If DefaultDip = Null Then
		def = 0
	Else If IsNumber(DefaultDip) Then
		def = DefaultDip
	Else
		def = TailwindSizeToDip(DefaultDip, 0)
	End If
	Return GetPropDip(Props, Key, def)
End Sub
...
Public Sub TailwindSizeToPx(Value As Object, DefaultPx As Float) As Float
	...
	If Value Is String Then
		...
		' Fractional tokens: 1/2, 1/3, 2/3, 3/4, etc.
		If s.Contains("/") Then
			Dim fracParts() As String = Regex.Split("/", s)
			If fracParts.Length = 2 Then
				Dim fracNum As Double = ParseCssNumber(fracParts(0).Trim, -1)
				Dim fracDen As Double = ParseCssNumber(fracParts(1).Trim, -1)
				If fracNum >= 0 And fracDen > 0 Then Return Max(0, DefaultPx * fracNum / fracDen)
			End If
		End If
...
' common resolver utilities added by standardization
Public Sub ResolvePxSizeSpec(SizeDip As Float) As String
    Dim px As Int = Max(1, Round(SizeDip / 1dip))
    Return px & "px"
End Sub

Public Sub ResolveWidthBase(Base As B4XView, DefaultValue As Float) As Float
    If Base.IsInitialized Then
        Dim parent As B4XView = Base.Parent
        If parent.IsInitialized And parent.Width > 0 Then Return parent.Width
        If Base.Width > 0 Then Return Base.Width
    End If
    Return DefaultValue
End Sub

Public Sub ResolveHeightBase(Base As B4XView, DefaultValue As Float) As Float
    If Base.IsInitialized Then
        Dim parent As B4XView = Base.Parent
        If parent.IsInitialized And parent.Height > 0 Then Return parent.Height
        If Base.Height > 0 Then Return Base.Height
    End If
    Return DefaultValue
End Sub

Public Sub ResolveTextSizeDip(Token As String) As Float
    Dim tm As Map = TailwindTextMetrics(Token, 14, 20)
    Return Max(1, tm.GetDefault("font_size", 14))
End Sub

''' <summary>
''' Measures a string using a canvas and returns the width with an optional buffer added.
''' This protects all callers from subtle clipping issues when laying out dynamic text.
''' </summary>
Public Sub MeasureTextWidthSafe(Text As String, TextSize As Float, tf As Typeface, BufferDip As Float) As Int
    If BufferDip = 0 Then BufferDip = 4dip ' default buffer
    Dim bmp As Bitmap
    bmp.InitializeMutable(1dip, 1dip)
    Dim cvs As Canvas
    cvs.Initialize2(bmp)
    Dim w As Int = cvs.MeasureStringWidth(Text, tf, TextSize)
    Return Max(0, w + BufferDip)
End Sub

Public Sub MeasureTextHeightSafe(Text As String, TextSize As Float, tf As Typeface, Width As Int, BufferDip As Float) As Int
    If BufferDip = 0 Then BufferDip = 4dip
    Dim lbl As Label
    lbl.Initialize("")
    lbl.Text = Text
    ...
    Dim h As Int = su.MeasureMultilineTextHeight(lbl, Text)
    Return Max(0, h + BufferDip)
End Sub
...
' Returns a glass spec map with opacity/border_opacity overridden by a size token.
' Size tokens: glass-xs (8%/8%), glass-sm (15%/12%), glass-md (30%/20%),
'              glass-lg (50%/35%), glass-xl (70%/50%). Empty or unrecognised = glass-md defaults.
Public Sub GetGlassSpecForSize(Size As String) As Map
	Dim spec As Map = GetGlassSpec
	Dim s As String = Size.ToLowerCase.Trim
	Dim opacity As Float
	Dim borderOpacity As Float
	Select Case s
		Case "glass-xs"
			opacity = 0.08
			borderOpacity = 0.08
		...
		Case "glass-2xl"
			...
			borderOpacity = 0.65
		Case Else
			' glass-md and any unrecognised value → DaisyUI defaults
			Return spec
	End Select
	spec.Put("opacity", opacity)
	spec.Put("border_opacity", borderOpacity)
	Return spec
End Sub

Public Sub ApplyGlassStyle(Target As B4XView, RadiusDip As Float, Size As String)
	' Approximate daisy .glass style for B4X views.
	' Size is a glass size token (glass-xs|glass-sm|glass-md|glass-lg|glass-xl|glass-2xl).
	' Pass "" or "glass-md" for the DaisyUI default.
	If Target.IsInitialized = False Then Return
	Dim spec As Map = GetGlassSpecForSize(Size)
...
' Apply background color with per-corner radii (B4A: uses GradientDrawable.setCornerRadii;
' other platforms fall back to a uniform max radius).
' Corner values in dip: TL=top-left, TR=top-right, BR=bottom-right, BL=bottom-left.
Public Sub SetColorPerCornerRadius(v As B4XView, BgColor As Int, TL As Float, TR As Float, BR As Float, BL As Float)
	#If B4A
	Dim gd As JavaObject
	gd.InitializeNewInstance("android.graphics.drawable.GradientDrawable", Null)
	gd.RunMethod("setColor", Array(BgColor))
	Dim radii() As Float = Array As Float(TL, TL, TR, TR, BR, BR, BL, BL)
	gd.RunMethod("setCornerRadii", Array(radii))
	Dim jo As JavaObject = v
	jo.RunMethod("setBackground", Array As Object(gd))
	#Else
	Dim maxR As Float = Max(TL, Max(TR, Max(BR, BL)))
	v.SetColorAndBorder(BgColor, 0, 0, maxR)
	#End If
End Sub
...
Public Sub AlphaColor(ColorValue As Int, Alpha01 As Float) As Int
...
Private Sub TryApplyGlassNative(Target As B4XView, RadiusDip As Float, BgOpacity As Float, BorderOpacity As Float) As Boolean
	Try
		...
		Dim strokeWidth As Int = Max(1dip, GetBorderDip(1dip))
		gd.RunMethod("setStroke", Array(strokeWidth, AlphaColor(xui.Color_White, BorderOpacity)))
...
'================================================================
' JOIN UTILITY HELPERS (daisyui join.css parity)
'================================================================
' Join groups buttons/inputs with merged borders and smart corners.
' Usage:
'   Dim joinSpec As Map = B4XDaisyVariants.GetJoinSpec("horizontal")
'   B4XDaisyVariants.ApplyJoinToContainer(pnlJoin, joinSpec)
'   B4XDaisyVariants.ApplyJoinItemToChild(btn, joinSpec, index, totalItems)
'================================================================

' Returns a join spec map with layout direction and radius rules.
' Orientation: "horizontal" (default), "vertical"
Public Sub GetJoinSpec(Orientation As String) As Map
	Dim m As Map
	m.Initialize
	Dim orient As String = Orientation.ToLowerCase.Trim
	If orient = "vertical" Then
		m.Put("orientation", "vertical")
	Else
		m.Put("orientation", "horizontal")
	End If
	m.Put("radius_field", GetRadiusFieldDip(8dip))
	m.Put("border_width", GetBorderDip(1dip))
	Return m
End Sub

' Apply join layout to the container panel (stores orientation in spec, not view).
Public Sub ApplyJoinToContainer(Container As B4XView, Spec As Map)
	' Placeholder: spec already contains orientation; container just needs to exist.
	If Container.IsInitialized = False Then Return
End Sub

' Apply join-item styling to a child view within a join group.
' Index is 0-based; Total is the number of join items in the group.
Public Sub ApplyJoinItemToChild(Item As B4XView, Spec As Map, Index As Int, Total As Int)
	If Item.IsInitialized = False Then Return
	If Spec.IsInitialized = False Then Return
	If Total <= 0 Then Return

	Dim orient As String = Spec.GetDefault("orientation", "horizontal")
	Dim radiusField As Float = Spec.GetDefault("radius_field", 8dip)
	Dim borderWidth As Float = Spec.GetDefault("border_width", 1dip)

	Dim tl As Float = 0, tr As Float = 0, br As Float = 0, bl As Float = 0
	Dim negMarginX As Int = 0, negMarginY As Int = 0

	If Total = 1 Then
		' Only child: fully rounded
		tl = radiusField: tr = radiusField: br = radiusField: bl = radiusField
	Else If Index = 0 Then
		' First child
		If orient = "vertical" Then
			tl = radiusField: tr = radiusField: br = 0: bl = 0
			negMarginY = 0
		Else
			tl = radiusField: tr = 0: br = 0: bl = radiusField
			negMarginX = 0
		End If
	Else If Index = Total - 1 Then
		' Last child
		If orient = "vertical" Then
			tl = 0: tr = 0: br = radiusField: bl = radiusField
			negMarginY = -Ceil(borderWidth)
		Else
			tl = 0: tr = radiusField: br = radiusField: bl = 0
			negMarginX = -Ceil(borderWidth)
		End If
	Else
		' Middle child: no rounded corners, negative margin to overlap
		tl = 0: tr = 0: br = 0: bl = 0
		If orient = "vertical" Then
			negMarginY = -Ceil(borderWidth)
		Else
	...
	Try
		...
		Dim bg As Int = 0
		Try
			Dim joItem As JavaObject = Item
			Dim drawable As JavaObject = joItem.RunMethod("getBackground", Null)
			If drawable.IsInitialized Then
				bg = drawable.RunMethod("getColor", Null)
			End If
		Catch
		End Try
		If bg = 0 Then bg = GetTokenColor("--color-base-100", xui.Color_White)
		gd.RunMethod("setColor", Array(bg))
		Dim strokeColor As Int = GetTokenColor("--color-base-300", xui.Color_RGB(203, 213, 225))
		gd.RunMethod("setStroke", Array(Ceil(borderWidth), strokeColor))
		Dim jo As JavaObject = Item
		jo.RunMethod("setBackground", Array(gd))
	Catch
		' Fallback: use B4X standard border
		Item.SetColorAndBorder(GetTokenColor("--color-base-100", xui.Color_White), Ceil(borderWidth), _
			GetTokenColor("--color-base-300", xui.Color_RGB(203, 213, 225)), radiusField)
	End Try
	#Else
	' B4i/B4J fallback: uniform radius (use max of the four)
	Dim maxR As Float = Max(Max(tl, tr), Max(br, bl))
	Item.SetColorAndBorder(GetTokenColor("--color-base-100", xui.Color_White), Ceil(borderWidth), _
		GetTokenColor("--color-base-300", xui.Color_RGB(203, 213, 225)), maxR)
	#End If

	' Apply negative margin for border overlap (B4A only for now).
	#If B4A
	If negMarginX <> 0 Or negMarginY <> 0 Then
		Try
			Dim lp As JavaObject = Item
			Dim parentObj As JavaObject = lp.RunMethod("getLayoutParams", Null)
			If parentObj.IsInitialized Then
				Dim curL As Int = parentObj.GetField("leftMargin")
				Dim curT As Int = parentObj.GetField("topMargin")
				parentObj.SetField("leftMargin", curL + negMarginX)
				parentObj.SetField("topMargin", curT + negMarginY)
				lp.RunMethod("setLayoutParams", Array(parentObj))
			End If
		Catch
		End Try
	End If
...
' Convenience: apply join to all children of a container automatically.
Public Sub ApplyJoinToAllChildren(Container As B4XView, Spec As Map)
	If Container.IsInitialized = False Then Return
	Dim total As Int = Container.NumberOfViews
	For i = 0 To total - 1
		Dim child As B4XView = Container.GetView(i)
		ApplyJoinItemToChild(child, Spec, i, total)
	Next
End Sub
...
Private Sub NormalizeSizeValueToken(Value As String) As String
	Dim s As String = NormalizeUtilityToken(Value.ToLowerCase)
	If s.Length = 0 Then Return ""
...
Private Sub HasExplicitCssSizeUnit(Token As String) As Boolean
	If Token = Null Then Return False
	Dim s As String = Token.Trim.ToLowerCase
	If s.Length = 0 Then Return False
	Return s.EndsWith("px") Or s.EndsWith("rem") Or s.EndsWith("em")
End Sub
...
Public Sub NormalizeRounded(Value As String) As String
	...
	Select Case s
		...
		Case "box", "rounded-box"
			Return "rounded-box"     ' DaisyUI --rounded-box: 1rem
		Case "field", "rounded-field"
			Return "rounded-field"   ' DaisyUI --rounded-field: 0.5rem
		Case "selector", "rounded-selector"
			Return "rounded-selector" ' DaisyUI --rounded-selector: 0.25rem
		Case Else
			Return "rounded-md"
	End Select
End Sub
...
Public Sub ResolveRoundedDip(Rounded As String, DefaultDip As Float) As Float
	Select Case NormalizeRounded(Rounded)
		...
		Case "rounded-full"
			Return 9999dip
		Case "rounded-box"
			Return GetRadiusBoxDip(IIf(DefaultDip > 0, DefaultDip, 16dip))  ' DaisyUI --radius-box (theme-aware)
		Case "rounded-field"
			Return GetRadiusFieldDip(IIf(DefaultDip > 0, DefaultDip, 8dip))   ' DaisyUI --radius-field (theme-aware)
		Case "rounded-selector"
			Return GetRadiusSelectorDip(IIf(DefaultDip > 0, DefaultDip, 4dip)) ' DaisyUI --radius-selector (theme-aware)
		Case Else
			Return DefaultDip
	End Select
End Sub
...
Public Sub DisableClipping(v As B4XView)
	' Disable clipping on a single view.  This is the canonical implementation used
	' throughout the library.  Components should call this directly rather than
	' re‑implement the logic themselves.
...
' Convenience helper that disables clipping on the supplied view and, optionally,
' a number of its ancestors.  Pass a negative value for MaxLevels to walk all the
' way to the root.
Public Sub DisableClippingChain(StartView As B4XView, MaxLevels As Int)
	If StartView.IsInitialized = False Then Return
	Dim v As B4XView = StartView
	Dim count As Int = 0
	Do While v.IsInitialized
		DisableClipping(v)
		' when walking up the tree the parent might be a ViewRootImpl or other
		' non-View object; attempting to access .Parent would crash when
		' the internal cast fails.  Wrap in Try and bail out if we cannot
		' obtain a usable parent.
		Try
			Dim pv As B4XView = v.Parent
			If pv.IsInitialized = False Then Exit
			v = pv
		Catch
			' parent not a view, stop climbing
			Exit
		End Try
		count = count + 1
		If MaxLevels >= 0 And count > MaxLevels Then Exit
	Loop
End Sub

' Recursively disable clipping on the view and all of its ancestors.  This mirrors
' the implementation previously duplicated in several components (e.g. tooltip).
Public Sub DisableClippingRecursive(v As B4XView)
	DisableClippingChain(v, -1)
End Sub
...
Public Sub ApplyElevation(v As B4XView, ShadowLevel As String)
	...
	Try
		...
		' Clip content to the rounded outline
		jo.RunMethod("setClipToOutline", Array(True))
		' Explicitly set BACKGROUND outline provider so the shadow follows the
		' corner radius of the background drawable on all Android API levels
		Try
			Dim ovp As JavaObject
			ovp.InitializeStatic("android.view.ViewOutlineProvider")
			jo.RunMethod("setOutlineProvider", Array As Object(ovp.GetField("BACKGROUND")))
			jo.RunMethod("invalidateOutline", Null)
		Catch
		End Try
	Catch
		Try
...
Public Sub ApplyFlexContainerTokens(fp As B4XDaisyFlexPanel, TokenString As String, DoRelayout As Boolean)
...
Public Sub ApplyParsedFlexContainerTokens(fp As B4XDaisyFlexPanel, Parsed As Map, DoRelayout As Boolean)
    If fp.IsInitialized = False Then Return
...
Public Sub ApplyFlexItemTokens(fp As B4XDaisyFlexPanel, v As B4XView, TokenString As String, DoRelayout As Boolean)
    If fp.IsInitialized = False Then Return
    If v.IsInitialized = False Then Return
...
Public Sub ApplyParsedFlexItemTokens(fp As B4XDaisyFlexPanel, v As B4XView, Parsed As Map, DoRelayout As Boolean)
    If fp.IsInitialized = False Then Return
    If v.IsInitialized = False Then Return
    If Parsed.IsInitialized = False Then Return
    
    Dim it As B4XDaisyFlexItem = fp.Item(v)
...
' Switches the global theme and forcefully updates all Daisy components on the given root view (e.g. Root of a B4XPage)
Public Sub ApplyThemeToPage(ThemeName As String, RootView As B4XView)
    ' 1. Update the global state
    SetActiveTheme(ThemeName)
    
    ' 2. Cascade the update to all views on the page
    UpdateThemeRecursive(RootView)
End Sub

Private Sub UpdateThemeRecursive(Parent As B4XView)
    For i = 0 To Parent.NumberOfViews - 1
        Dim v As B4XView = Parent.GetView(i)
        
        ' Check if this view is a Daisy component wrapper
        If v.Tag <> Null Then
            ' Duck-typing: Check if the custom class instance has an UpdateTheme method
            If xui.SubExists(v.Tag, "UpdateTheme", 0) Then
                CallSub(v.Tag, "UpdateTheme")
            End If
        End If
        
        ' Recursively search children (for panels, layouts, etc)
        If v.NumberOfViews > 0 Then
            UpdateThemeRecursive(v)
        End If
    Next
End Sub

' Centralized helper to apply dashed/dotted/solid borders using native Android drawables where needed.
' Supports B4A (Native Dash/Dot), B4i/B4J (Fallbacks to Solid).
Public Sub ApplyDashedBorder(Target As B4XView, FillColor As Int, BorderWidth As Float, BorderColor As Int, Radius As Float, Style As String)
    #If B4A
    Try
        Dim bw As Int = Max(0, BorderWidth)
        Dim s As String = IIf(Style = Null, "solid", Style.ToLowerCase.Trim)
        If s = "dashed" Or s = "dash" Or s = "dotted" Or s = "dot" Then
            Dim dashW As Float = 0
            Dim dashGap As Float = 0
            If s = "dashed" Or s = "dash" Then
                dashW = Max(4dip, bw * 3)
                dashGap = Max(3dip, bw * 2)
            Else If s = "dotted" Or s = "dot" Then
                dashW = Max(2dip, bw)
                dashGap = Max(2dip, bw)
            End If
            ...
            ' Use setCornerRadius (single float) for GradientDrawable.getOutline() compatibility
            ' across all Android API levels — setCornerRadii falls back to rect outline on < API 31
            gd.RunMethod("setCornerRadius", Array(Radius))
            
            If bw > 0 Then
                gd.RunMethod("setStroke", Array(bw, BorderColor, dashW, dashGap))
            End If
            
            Dim jo As JavaObject = Target
            jo.RunMethod("setBackground", Array(gd))
            
            ' For larger components with children at edges (like Card or Collapse), 
            ' the children might cover the border. One could add padding here if needed,
            ' but we keep it identical to Badge/Button for now.
        Else
            ' Build a GradientDrawable explicitly so setCornerRadius(float) is used
            ' (single-value call).  B4X's SetColorAndBorder uses setCornerRadii(float[])
            ' internally; when that array form is used, GradientDrawable.getOutline()
            ' falls back to setConvexPath which the Android shadow renderer does NOT
            ' support — making the elevation shadow rectangular even on rounded buttons.
            ' Using setCornerRadius(float) ensures getOutline() calls outline.setRoundRect()
            ' so shadows correctly follow the button's corner radius.
            Dim gdSolid As JavaObject
            gdSolid.InitializeNewInstance("android.graphics.drawable.GradientDrawable", Null)
            gdSolid.RunMethod("setShape", Array(0)) 'RECTANGLE
            gdSolid.RunMethod("setColor", Array(FillColor))
            gdSolid.RunMethod("setCornerRadius", Array(Radius))
            If bw > 0 Then
                gdSolid.RunMethod("setStroke", Array(bw, BorderColor))
            End If
            Dim joTarget As JavaObject = Target
            joTarget.RunMethod("setBackground", Array(gdSolid))
        End If
    Catch
        Target.SetColorAndBorder(FillColor, BorderWidth, BorderColor, Radius)
    End Try
    #Else
    Target.SetColorAndBorder(FillColor, BorderWidth, BorderColor, Radius)
    #End If
...
''' <summary>
''' Shifts all sibling views whose Top > View.Top by Delta dip, and adjusts the parent
''' panel height accordingly. Used by Accordion and standalone Collapse to auto-restack
''' page siblings after an expand/collapse without requiring page-level relayout code.
''' AnimDuration: 0 for instant, > 0 for animated.
''' </summary>
Public Sub ShiftSiblingsBelow(View As B4XView, Delta As Int, AnimDuration As Int)
    ...
    For i = 0 To parent.NumberOfViews - 1
        Dim v As B4XView = parent.GetView(i)
        If v <> View And v.Top > View.Top Then
            v.SetLayoutAnimated(AnimDuration, v.Left, v.Top + Delta, v.Width, v.Height)
        End If
    Next
