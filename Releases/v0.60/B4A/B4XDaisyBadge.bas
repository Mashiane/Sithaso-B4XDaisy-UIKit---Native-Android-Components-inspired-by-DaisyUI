...
Private Sub ApplyDesignerProps(Props As Map)
	...
	mText = B4XDaisyVariants.GetPropString(Props, "Text", "Badge")
	...
	mClosable = B4XDaisyVariants.GetPropBool(Props, "Closable", False)
	mToggle = B4XDaisyVariants.GetPropBool(Props, "Toggle", False)
	mChecked = B4XDaisyVariants.GetPropBool(Props, "Checked", False)
	...
	mIconAsset = B4XDaisyVariants.GetPropString(Props, "IconAsset", "")
	mAvatarPosition = B4XDaisyVariants.GetPropString(Props, "AvatarPosition", "left")
	mAvatarVisible = B4XDaisyVariants.GetPropBool(Props, "AvatarVisible", False)
	mAvatarImage = B4XDaisyVariants.GetPropString(Props, "AvatarImage", "").Trim ' Kept original mAvatarImage as it was not replaced by mAvatarAsset
	mAvatarText = B4XDaisyVariants.GetPropString(Props, "AvatarText", "") ' Kept original mAvatarText as it was not replaced by mAvatarAsset
	...
	mBorderColor = B4XDaisyVariants.GetPropColor(Props, "BorderColor", 0)
	' ApplyDesignerSpacing(mPadding, mMargin) ' This line was not in the original code, so it's commented out or removed.

	mBadgeStyle = B4XDaisyVariants.NormalizeBadgeStyle(B4XDaisyVariants.GetPropString(Props, "BadgeStyle", "solid")) ' Moved from earlier in the block
	mTextCentered = B4XDaisyVariants.GetPropBool(Props, "TextCentered", mTextCentered) ' Moved from earlier in the block
	mRoundedBox = B4XDaisyVariants.GetPropBool(Props, "RoundedBox", True) ' Kept original
	...
	mId = B4XDaisyVariants.GetPropString(Props, "ID", "") ' Kept original
	mCloseIconAsset = B4XDaisyVariants.GetPropString(Props, "CloseIconAsset", "xmark-solid.svg").Trim
...
Private Sub LayoutContent(ContentRect As B4XRect)
	...
	Dim textValue As String = B4XDaisyVariants.NormalizeSingleLineText(mText)
...
Private Sub EstimatePreferredWidth As Int
	...
	Dim gap As Int = 8dip
	Dim textW As Int = MeasureSingleLineWidth(B4XDaisyVariants.NormalizeSingleLineText(mText), f)
