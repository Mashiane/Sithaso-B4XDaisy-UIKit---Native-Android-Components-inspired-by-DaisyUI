...
Private Sub ApplyDesignerProps(Props As Map)
	...
	mTextSize = ResolveTextSize(GetPropObject(Props, "TextSize", "text-sm"), mTextSize)
	mFontBold = GetPropBool(Props, "FontBold", False)
	mSingleLine = GetPropBool(Props, "SingleLine", True)
	mHAlign = NormalizeHAlign(GetPropString(Props, "HAlign", "LEFT"))
	mVAlign = NormalizeVAlign(GetPropString(Props, "VAlign", "CENTER"))
	mPadding = Max(0, GetPropFloat(Props, "Padding", 0) * 1dip)
	mMargin = GetPropString(Props, "Margin", "")
	mRoundedBox = GetPropBool(Props, "RoundedBox", False)
	mBorderWidth = Max(0, GetPropFloat(Props, "BorderWidth", 0) * 1dip)
	mBorderColor = GetPropColor(Props, "BorderColor", 0x00000000)
	mVisible = GetPropBool(Props, "Visible", True)
	mEnabled = GetPropBool(Props, "Enabled", True)
...
Private Sub GetPropSizeDip(Props As Map, Key As String, DefaultDipValue As Object) As Float
	...
	Return B4XDaisyVariants.TailwindSizeToDip(Props.Get(Key), baseDip)
End Sub
