...
Private Sub ApplyDesignerProps(Props As Map)
    ...
    mWidth = Max(1dip, GetPropSizeDip(Props, "Width", "10"))
    mHeight = Max(1dip, GetPropSizeDip(Props, "Height", "10"))
	mPadding = GetPropString(Props, "Padding", "")
	mMargin = GetPropString(Props, "Margin", "")
    mBackgroundColor = GetPropColor(Props, "BackgroundColor", 0x00FFFFFF)
    mTextColor = GetPropColor(Props, "TextColor", 0xFF000000)
    mTextSize = ResolveTextSize(GetPropString(Props, "TextSize", "text-sm"))
    mText = GetPropString(Props, "Text", "")
    mRounded = GetPropString(Props, "Rounded", "none")
    mRoundedBox = GetPropBool(Props, "RoundedBox", False)
    mShadow = B4XDaisyVariants.NormalizeShadow(GetPropString(Props, "Shadow", "none"))
     mPlaceContentCenter = GetPropBool(Props, "PlaceContentCenter", False)
     mBorderWidth = GetPropInt(Props, "BorderWidth", 0)
     mBorderColor = GetPropColor(Props, "BorderColor", 0xFF000000)
     mBorderStyle = NormalizeBorderStyle(GetPropString(Props, "BorderStyle", "solid"))
     mBorderReliefStrength = ClampReliefStrength(GetPropInt(Props, "BorderReliefStrength", 55))
     mAutoReliefByStyle = GetPropBool(Props, "AutoReliefByStyle", True)
     mIsSkeleton = GetPropBool(Props, "IsSkeleton", False)
