...
Private Sub ApplyDesignerProps(Props As Map)
	...
	mColumns = Max(1, GetPropInt(Props, "ColumnsPerPage", 4))
	mRowsPerPage = Max(1, GetPropInt(Props, "RowsPerPage", 6))
	mAutoGrid = GetPropBoolean(Props, "AutoGrid", False)
	mMinCellWidth = Max(1dip, GetPropInt(Props, "MinCellWidth", 72) * 1dip)
	mMinCellHeight = Max(1dip, GetPropInt(Props, "MinCellHeight", 100) * 1dip)
	mPagePadding = Max(0, GetPropInt(Props, "PagePadding", 16) * 1dip)
	mCellSpacing = Max(0, GetPropInt(Props, "CellSpacing", 12) * 1dip)
	mCellSpacingY = Max(0, GetPropInt(Props, "CellSpacingY", 0) * 1dip)
	mGridTopOffset = Max(0, GetPropInt(Props, "GridTopOffset", 12) * 1dip)
	mActiveIndicatorColor = GetPropColor(Props, "ActiveIndicatorColor", 0xFF3B82F6)
	mInactiveIndicatorColor = GetPropColor(Props, "InactiveIndicatorColor", 0x553B82F6)
	mTextColor = GetPropColor(Props, "TextColor", 0xFFFFFFFF)
	mWidthSpec = NormalizeSizeSpec(GetPropString(Props, "Width", "100%"), "100%")
	mHeightSpec = NormalizeSizeSpec(GetPropString(Props, "Height", "100%"), "100%")
	setBackgroundImage(GetPropString(Props, "BackgroundImage", ""))
End Sub
