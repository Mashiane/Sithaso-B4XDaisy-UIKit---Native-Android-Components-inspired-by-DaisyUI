...
Public Sub ScrollFocusedInputIntoView
    ...
    Catch
        Log("B4XPageScrollDemo.ScrollFocusedInputIntoView: " & LastException.Message)
    End Try
End Sub
...
Private Sub HandleInputFocus(bHasFocus As Boolean)
    Try
        ...
        End If
    Catch
        Log("B4XPageScrollDemo.HandleInputFocus: " & LastException.Message)
    End Try
End Sub

Private Sub txtEmail_FocusChanged(bHasFocus As Boolean)
    HandleInputFocus(bHasFocus)
End Sub

Private Sub txtPassword_FocusChanged(bHasFocus As Boolean)
    HandleInputFocus(bHasFocus)
End Sub
#End If
